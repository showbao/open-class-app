...
st.title("✏️ 登記公開觀課")
st.caption("填寫完畢後，資料將自動儲存至系統，並顯示於行事曆。")
st.divider()
...
with st.form("register_form"):
    ...
    with st.expander(f"📌 {indicator_labels[selected_indicator]} — 子項目預覽（共 {len(sub_items)} 項）"):
...
if submitted:
    if not unit.strip():
        ...
        st.info("前往「我的觀課行事曆」可查看已登記場次。")
        st.page_link("pages/2_我的行事曆.py", label="📅 前往行事曆")
